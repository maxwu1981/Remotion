#!/usr/bin/env node
/**
 * Voiceover for 生活應用系列 EP05「我怎麼讓 Pinterest 自動把畫作變流量」。
 * 全程曉晴(第一人稱老闆視角) · 故事解說片。
 *
 * 逐字來源＝單一真相：src/videos/_explainer/specs/pinterest-autopost.json 的 script。
 * 每句 → public/vo/pinterest-autopost/<id>.mp3
 * 量到的秒數 → src/videos/_explainer/specs/pinterest-autopost.vo.json
 *
 *   node scripts/make-vo-pinterest-autopost.mjs            # 全部
 *   node scripts/make-vo-pinterest-autopost.mjs rj3        # 只重生指定句(過稿改字用)
 */
...
const specPath = join(root, "src", "videos", "_explainer", "specs", "pinterest-autopost.json");
const voDir = join(root, "public", "vo", "pinterest-autopost");
const manifestPath = join(root, "src", "videos", "_explainer", "specs", "pinterest-autopost.vo.json");
 ...
 *  「重開」在此是 chóng-kāi(再開一個)，edge-tts 常誤唸 zhòng → 換同音單音字「崇」。
...
console.log("engine: edge-tts  曉晴旁白(Pinterest 自動發文 EP05)\n");
for (const { id, text } of LINES) {
  if (only.length && !only.includes(id)) continue;
  const raw = join("/tmp", `${id}.pa.edge.mp3`);
