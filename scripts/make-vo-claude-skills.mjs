#!/usr/bin/env node
/**
 * Voiceover for the "Claude Code Skills 實戰" explainer — Microsoft edge-tts neural
 * voice (free, no key, needs network). Reads the canonical lines from
 * src/videos/claude-code-skills/script.json (same text the scenes caption),
 * writes one mp3 per id into public/vo/claude-code-skills/<id>.mp3, and records
 * measured durations in src/videos/claude-code-skills/vo-manifest.json so every
...
const voDir = join(root, "public", "vo", "claude-code-skills");
const videoDir = join(root, "src", "videos", "claude-code-skills");
...
for (const [id, text] of CUES) {
  const raw = join("/tmp", `${id}.ccs.edge.mp3`);
...
console.log(`\n✓ ${CUES.length} clips → public/vo/claude-code-skills/  ·  manifest → src/videos/claude-code-skills/vo-manifest.json`);
