...
# CLAUDE.md（Remotion 專案）

通用行為準則（Think Before Coding／Simplicity First／Surgical Changes／Goal-Driven Execution／GEO-First Publishing）在全域 `~/.claude/CLAUDE.md` §1–§5，兩份會一起載入，此處不重複。本檔只放**本專案的影片製作鐵則**。影片產線總覽見 repo 根目錄 `VIDEO-PRODUCTION.md`。

## 6. 影片製作鐵則（Remotion · 本專案）

**用 Remotion 做任何影片時，這些是硬規則，不是建議。**（編號沿用「§6」以維持既有記憶/skill 的交叉引用。）
...
- **標題/描述**：套全域 `CLAUDE.md §5` GEO（problem→solution＋具體結果、開字幕、章節時間戳、實體講清楚）。
