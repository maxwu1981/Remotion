...
import spec from "../_explainer/specs/ig-autopost.json";
import voManifest from "../_explainer/specs/ig-autopost.vo.json";
import { WhiteBg, ACC, darkCard, Glare, hairline, emberClip, emberPill, rgba, accOf } from "./glass";

/**
 * ig-autopost 黑曜石精修版（16:9, 1920×1080, 30fps）— 生活應用系列 EP06。
 * 沿用 EP04（fb-autopost）同一套黑曜石模板：讀 _explainer/specs/ig-autopost.json（待過稿）
 *   + 同名 vo.json + 曉晴 VO(public/vo/ig-autopost)。只換視覺皮＝淺底畫布 + 黑曜石深卡玻璃
 *   + 橙漸層鉤子 + 概念錨頂帶（系列一致）。
 * HAS_VO：VO 尚未生成時設 false，跳過音軌讓骨架可預覽；旁白定稿、生完 edge-tts 後改 true。
 */
const FPS = 30;
const HAS_VO = true;
...
const ANCHORS: (React.ReactNode | null)[] = [
  ...
  <>五個零件，<OS>多一步公開網址</OS></>,
  <>資格 = <OS>商業帳號 + 連粉專</OS></>,
  <>Token <OS>沿用</OS>，多勾 2 個 IG 權限</>,
  <>IG 只吃<OS>公開網址</OS>，不吃本機檔</>,
  <>發文 = <OS>建容器 → 發布</OS></>,
  <>自動 = <OS>一支排程發兩平台</OS></>,
  <>去重＋回收 = <OS>天天不斷更</OS></>,
  <>24 小時<OS>上限 100 篇</OS></>,
  <><OS>架構已驗證</OS>，接上就發</>,
  <>公開 URL · 商業帳號 · <OS>連結不可點</OS></>,
  null,
  null,
];
    ...
    <AbsoluteFill style={{ fontFamily: FONT.uiCjk, color: ACC.ink }}>
      <div style={{ position: "absolute", top: 62, left: 96, display: "flex", alignItems: "center", gap: 22, opacity: o }}>
        <div style={{ ...darkCard(ACC.ember, { r: 999, glow: 0.5 }), padding: "10px 28px", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 30, letterSpacing: 2, color: "#fff" }}>生活應用系列 EP05</span>
        </div>
      ...
      <div style={{ position: "absolute", top: 150, right: 92, width: 620, ...darkCard(ACC.ember, { r: 40, glow: 1.2 }), overflow: "hidden", padding: "40px 42px", transform: `rotate(-2.5deg)`, opacity: o }}>
        ...
        <div style={{ position: "relative", padding: "22px 24px", borderRadius: 16, background: rgba("#060a14", 0.7), border: `1px solid ${rgba(ACC.ember, 0.35)}`, boxShadow: `inset 0 2px 6px ${rgba("#000", 0.5)}` }}>
          <span style={{ fontFamily: FONT.monoCjk, fontSize: 30, lineHeight: 1.42, color: "#fff" }}>
            <span style={{ color: ACC.gold }}>$</span> 「Hi Claude，沿用同一把 Token，讓我的 IG 也<span style={{ color: ACC.emberSoft }}>每天自動發文</span>」
          </span>
        </div>
        ...
        <div style={{ position: "relative", display: "flex", flexDirection: "column", gap: 15 }}>
          {["素材夾 → 圖上公開網址 → IG API → 貼文", "沿用 FB 的 Token · 官方 API · 免月費", "一支排程 · FB + IG 一起自動發"].map((t, i) => (
    ...
    <AbsoluteFill>
      ...
      {HAS_VO && marks.map((m: any) => (
        <Sequence key={`a-${m.id}`} from={m.start}>
          <Audio src={staticFile(`vo/ig-autopost/${m.id}.mp3`)} />
        </Sequence>
      ))}
    </AbsoluteFill>
  );
...
export const IgAutopostThumb: React.FC = () => (
  <AbsoluteFill>
    <WhiteBg />
...
export const IgAutopostObsidian: React.FC = () => (
  <AbsoluteFill>
    <WhiteBg />
    <Audio loop src={staticFile("bgm-ig-autopost.mp3")} volume={0.11} />
    <TransitionSeries>
      {SCENES.flatMap((s, i) => {
        const seq = (
