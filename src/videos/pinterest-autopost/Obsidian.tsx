...
import spec from "../_explainer/specs/pinterest-autopost.json";
import voManifest from "../_explainer/specs/pinterest-autopost.vo.json";
import { WhiteBg, ACC, darkCard, Glare, hairline, emberClip, emberPill, rgba, accOf } from "./glass";

/**
 * pinterest-autopost 黑曜石精修版（16:9, 1920×1080, 30fps）。
 * 生活應用系列 EP05：我怎麼讓 Pinterest 自動把畫作變流量（故事解說片）。
 * 資料驅動：讀 _explainer/specs/pinterest-autopost.json（已過稿）+ 同名 vo.json + 曉晴 VO(public/vo/pinterest-autopost)。
 * 視覺＝系列黑曜石皮（淺底畫布 + 黑曜石深卡玻璃 + 橙漸層鉤子 + 概念錨頂帶）。
 */
const FPS = 30;
...
/* ── 每景概念錨（index 對 scene；null=不顯示：封面/outro）── */
...
const ANCHORS: (React.ReactNode | null)[] = [
  null,
  <>Pin = <OS>長效帶客</OS>，不是一次性曝光</>,
  <>畫作 → <OS>帶連結 Pin</OS> → 導流</>,
  <>被拒真因 = <OS>名稱不一致</OS></>,
  <>把案子推到 <OS>真人複審</OS> 手上</>,
  <>發 Pin = <OS>三步 API</OS></>,
  <>合規 = <OS>帳號的資產</OS></>,
  <>公開發 = <OS>要升 Standard</OS></>,
  <>審查時，<OS>畫作沒停過</OS></>,
  <>官方 API = <OS>主導權在自己</OS></>,
  <><OS>33 張</OS> ＋ 全鏈路驗證</>,
  null,
...
export const pinterestObsidianFrames = () =>
    ...
    <AbsoluteFill style={{ fontFamily: FONT.uiCjk, color: ACC.ink }}>
      <div style={{ position: "absolute", top: 62, left: 96, display: "flex", alignItems: "center", gap: 22, opacity: o }}>
        <div style={{ ...darkCard(ACC.ember, { r: 999, glow: 0.5 }), padding: "10px 28px", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 30, letterSpacing: 2, color: "#fff" }}>生活應用系列 EP06</span>
        </div>
      ...
      <div style={{ position: "absolute", top: 150, right: 92, width: 620, ...darkCard(ACC.ember, { r: 40, glow: 1.2 }), overflow: "hidden", padding: "40px 42px", transform: `rotate(-2.5deg)`, opacity: o }}>
        ...
        <div style={{ position: "relative", padding: "22px 24px", borderRadius: 16, background: rgba("#060a14", 0.7), border: `1px solid ${rgba(ACC.ember, 0.35)}`, boxShadow: `inset 0 2px 6px ${rgba("#000", 0.5)}` }}>
          <span style={{ fontFamily: FONT.monoCjk, fontSize: 30, lineHeight: 1.42, color: "#fff" }}>
            <span style={{ color: ACC.gold }}>$</span> 「Hi Claude，幫我用 Pinterest 官方 API <span style={{ color: ACC.emberSoft }}>自動發我的畫作</span>」
          </span>
        </div>
        ...
        <div style={{ position: "relative", display: "flex", flexDirection: "column", gap: 15 }}>
          {["原創畫作 → 自動組 Pin → 帶連結導 Shopify", "Pinterest 官方 v5 API · 內建合規護欄", "33 張 Pin 鋪滿 4 看板 · 全鏈路已驗證"].map((t, i) => (
    ...
    <AbsoluteFill>
      ...
      {marks.map((m: any) => (
        <Sequence key={`a-${m.id}`} from={m.start}>
          <Audio src={staticFile(`vo/pinterest-autopost/${m.id}.mp3`)} />
        </Sequence>
      ))}
    </AbsoluteFill>
  );
...
export const PinterestObsidian: React.FC = () => (
  <AbsoluteFill>
    <WhiteBg />
    {/* 專屬水晶鋼琴 BGM（Gemini Lyria 3 生，59s loop 墊全片，低音量不搶旁白）*/}
    <Audio src={staticFile("bgm-pinterest-autopost.mp3")} loop volume={0.12} />
    <TransitionSeries>
      {SCENES.flatMap((s, i) => {
        const seq = (
