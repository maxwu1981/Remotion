...
/**
 * 「生活應用」黑曜石視覺系統 — fb-autopost（FB 每天自動發文）。
 * 沿用系列黑曜石 v3/v4（LinkedIn EP01 / YouTube EP02）：
 *   淺底畫布 + 黑曜石玻璃深卡（hairline 邊 + 收斂雙層光暈 + 接地陰影）+ 橙漸層鉤子字。
 * 本片背景=中性高級冷調（雪白→霧藍→淺鋼藍）+ 極淡「貼文卡」水印（呼應社群發文，不搶前景）。
...
export const ACC = {
  ...
  mint: "#10B981", // 自動化成功
  violet: COLORS.hi.violet, // 判斷/人工
  rose: "#F43F5E", // 坑/警示
  gold: "#F5B942", // 深卡上的高亮黃（數字）
  fb: "#1877F2", // Facebook 藍（背景點綴 + 品牌）
  fbSoft: "#5B9BF6",
};

/** 語意 accent key → 色（對映 spec 的 AccentKey）。 */
export const accOf = (k?: string): string =>
  ({ blue: ACC.fb, green: ACC.mint, warn: ACC.rose, violet: ACC.violet, claude: ACC.ember, teal: ACC.signal }[k ?? "claude"] ?? ACC.ember);

/** 極淡「貼文卡」水印（16:9 縮圖 + 兩行文字線，低對比，呼應社群發文）。 */
const PostBackdrop: React.FC = () => {
  const cards: [number, number, number][] = [
    [50, 82, 250], [40, 430, 236], [64, 742, 244],
    [1656, 70, 244], [1642, 404, 256], [1666, 752, 236],
    [336, 858, 230], [1356, 862, 236],
  ];
  return (
      ...
      {cards.map(([x, y, w], i) => {
        ...
        return (
          <div key={i} style={{ position: "absolute", left: x, top: y, width: w }}>
            <div style={{ width: w, height: th, borderRadius: 14, background: `linear-gradient(150deg, ${rgba(ACC.fb, 0.09)}, ${rgba(ACC.ember, 0.06)})`, border: `1px solid ${rgba(ACC.ink, 0.055)}`, boxShadow: `0 10px 24px ${rgba(ACC.ink, 0.05)}` }} />
            <div style={{ display: "flex", gap: 9, marginTop: 9 }}>
              <div style={{ width: 22, height: 22, borderRadius: 999, background: rgba(ACC.ink, 0.08), flexShrink: 0 }} />
              <div style={{ flex: 1 }}>
                <div style={{ height: 8, borderRadius: 4, background: rgba(ACC.ink, 0.08), marginBottom: 7 }} />
                <div style={{ height: 7, width: "60%", borderRadius: 4, background: rgba(ACC.ink, 0.055) }} />
              </div>
            </div>
          </div>
        );
...
/** 中性高級冷調畫布 + 貼文卡水印 + 多顆低調收斂光暈（靜態防抖）。全片墊底。 */
export const WhiteBg: React.FC = () => (
  <AbsoluteFill style={{ background: "linear-gradient(180deg, #FAFCFF 0%, #EEF3FB 52%, #E4EAF4 100%)" }}>
    ...
    <div style={{ position: "absolute", left: 1200, top: 60, width: 600, height: 600, borderRadius: "50%", background: rgba(ACC.fb, 0.06), filter: "blur(150px)" }} />
