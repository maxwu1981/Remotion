...
/* ───────────────────────── Roadmap — 全集地圖（四站） ───────────────────────
 * A course-style map shown right after the cover so a viewer sees the whole
 * journey before it starts. The 4th stop teases the 💎 payoff to earn retention.
 */
export const ROADMAP = [
  { n: 1, emoji: "🧩", zh: "先搞懂是什麼", sub: "commit／push 是 Git 的功能", color: MOTIF.commit },
  { n: 2, emoji: "🕐", zh: "何時用、怎麼做", sub: "觸發點 × 三種輸入方式", color: MOTIF.file },
  { n: 3, emoji: "🔧", zh: "手把手 × 避坑", sub: "桌面版一句話 + 常見錯誤", color: MOTIF.chat },
...
/* ───────────────────────── 💎 今日提示詞（挖寶段 / 固定欄目） ──────────────────
 * The signature "take-away" segment: a collectible, copy-paste prompt viewers
 * can dig out of every episode. Dark card = "乾貨來了" contrast against the light
 * canvas. Rationale + a variant turn a snippet into transferable knowledge; the
 * 資訊欄 note doubles as a GEO signal (AI engines cite the described text).
 */
export const GEM = {
  label: "今日提示詞",
  labelEn: "Prompt of the Day",
  hint: "在 Claude Code／Cowork 的聊天框，直接貼這一句",
  prompt: "幫我把目前的修改 commit 並 push 到 GitHub，commit 說明寫「這次改了什麼」。",
  why: "一句話同時交代『做什麼』＋『commit 說明』，Claude 不用反問，一步到位。",
  variant: "只想先存檔、還不想上傳？把句尾改成「先 commit 就好，先不要 push」。",
