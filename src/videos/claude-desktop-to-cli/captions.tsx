 ...
 * mp3 per cue id into public/vo/claude-desktop-to-cli/ and fill this manifest.
 */
 ...
 * long line gets the seconds it needs to be read at a calm pace and a short
 * line isn't left lingering. Replaced per-line by the measured VO length once
 ...
 * fixed fallback while captions-only), so narration never overruns the next
 * line. Returns the timed cues plus the scene length they imply (floored by
 * `minDur` so late visual beats always have room).
 */
...
/** CaptionTrack pre-bound to this video's VO manifest + public/vo/claude-desktop-to-cli/. */
export const Captions: React.FC<{ cues: Cue[] }> = ({ cues }) => {
  const hideBars = React.useContext(ReelCaptionContext);
  return <Base cues={cues} vo={VO} voDir="vo/claude-desktop-to-cli" hideBars={hideBars} />;
};
