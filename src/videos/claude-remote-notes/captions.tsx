...
/** Measured voiceover lengths (seconds). Written by scripts/make-vo-claude-remote.mjs. */
...
const FALLBACK = 3.0; // per-line seconds when no VO exists yet (captions-only)
...
/** CaptionTrack pre-bound to this video's VO manifest + public/vo/claude-remote-notes/. */
export const Captions: React.FC<{ cues: Cue[] }> = ({ cues }) => {
  const hideBars = React.useContext(ReelCaptionContext);
  return <Base cues={cues} vo={VO} voDir="vo/claude-remote-notes" hideBars={hideBars} />;
};
