...
/**
 * ExplainerObsidian（_explainer 模板家族）用的黑曜石視覺系統 —
 * 與各片 <name>/glass.tsx 同一套精修語言，這份是 _explainer 通用模板自己的複本。
...
/** 語意 accent key（對映 spec 的 AccentKey）→ 色。 */
export const accOf = (k?: string): string =>
  ({ blue: ACC.signal, green: ACC.mint, warn: ACC.rose, violet: ACC.violet, claude: ACC.ember, teal: ACC.signal }[k ?? "claude"] ?? ACC.ember);
