...
/**
 * 「生活應用 EP01 · LinkedIn 全自動翻新」視覺系統 — 2026-07-03 高級感精修版:
 * 白底畫布 + 黑曜石玻璃卡(hairline 邊 + 收斂雙層光暈 + 接地陰影) + 橙漸層鉤子字。
 * (v1 銀玻璃拉絲金屬版退役;本版與實戰自動化 EP01 同一套精修語言,LinkedIn 藍只當碑內主題色。)
...
export const ACC = {
  ...
  mint: "#10B981", // 成果/After(深卡上要亮)
  violet: COLORS.hi.violet, // 人工/判斷
  rose: "#F43F5E", // 慘況/盲點警示(深卡上要亮)
  gold: "#F5B942", // 深卡上的高亮黃(數字)
  linkedin: "#3B82F6", // LinkedIn 主題藍(只在檔案卡內當點綴,亮階好讀)
};
...
/** 遮蔽馬賽克塊(個資位置的像素格,靜態)。 */
export const Mosaic: React.FC<{ w: number; h: number; cell?: number; style?: React.CSSProperties }> = ({ w, h, cell = 14, style }) => (
  <div
    style={{
      width: w,
      height: h,
      borderRadius: 6,
      background: [
        `repeating-linear-gradient(0deg, ${rgba("#ffffff", 0.16)} 0px, ${rgba("#ffffff", 0.16)} ${cell}px, ${rgba("#ffffff", 0.07)} ${cell}px, ${rgba("#ffffff", 0.07)} ${cell * 2}px)`,
        `repeating-linear-gradient(90deg, ${rgba("#ffffff", 0.14)} 0px, ${rgba("#ffffff", 0.14)} ${cell}px, ${rgba("#ffffff", 0.04)} ${cell}px, ${rgba("#ffffff", 0.04)} ${cell * 2}px)`,
      ].join(", "),
      ...style,
    }}
  />
