...
import voManifest from "../_explainer/specs/remotion-autovideo.vo.json";
import { WhiteBg, ACC, darkCard, Glare, hairline, emberClip, rgba, accOf } from "../fb-autopost/glass";

/**
 * remotion-autovideo 黑曜石版（16:9, 1920×1080, 30fps）— 生活應用 EP05。
 * 資料驅動：讀 _explainer/specs/remotion-autovideo.json（已過稿）+ 同名 vo.json + 曉晴 VO(public/vo/remotion-autovideo)。
 * 復用 fb-autopost/glass 深色玻璃皮 + 橙漸層鉤子 + 概念錨頂帶（生活應用系列一致）。
 */
const FPS = 30;
...
export const ravObsidianFrames = () =>
    ...
    <AbsoluteFill style={{ fontFamily: FONT.uiCjk, color: ACC.ink }}>
      <div style={{ position: "absolute", top: 62, left: 96, display: "flex", alignItems: "center", gap: 22, opacity: o }}>
        <div style={{ ...darkCard(ACC.ember, { r: 999, glow: 0.5 }), padding: "10px 28px", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 30, letterSpacing: 2, color: "#fff" }}>生活應用系列</span>
        </div>
      ...
      <div style={{ position: "absolute", top: 194, left: 96, opacity: o, transform: `translateY(${(1 - o) * 18}px)` }}>
        ...
        <div style={{ ...emberClip, background: "linear-gradient(160deg, #F09A6B 0%, #D97757 42%, #B54C29 100%)", WebkitBackgroundClip: "text", backgroundClip: "text", fontWeight: 800, fontSize: 132, letterSpacing: -2, lineHeight: 1.02, marginTop: 6, filter: `drop-shadow(0 8px 20px ${rgba(ACC.ember, 0.35)})` }}>{s.titlePost}</div>
    ...
    <AbsoluteFill>
      ...
      {marks.map((m: any) => (
        <Sequence key={`a-${m.id}`} from={m.start}>
          <Audio src={staticFile(`vo/remotion-autovideo/${m.id}.mp3`)} />
        </Sequence>
      ))}
    </AbsoluteFill>
  );
...
export const RemotionAutoVideoObsidian: React.FC = () => (
  <AbsoluteFill>
    <WhiteBg />
    <TransitionSeries>
      {SCENES.flatMap((s, i) => {
