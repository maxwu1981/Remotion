...
const { cues: CUES, dur: DUR } = buildScene(
  ["ot-c1", "ot-c2", "ot-c3", "ot-c4", "ot-c5", "ot-c6"],
  { lead: 14, tail: 56, minDur: 600 },
);
const at = (i: number) => CUES[i].from;

const ACCENTS = [MOTIF.node, MOTIF.perm, MOTIF.path, MOTIF.auth];
const EMOJIS = ["🟢", "🔑", "🧭", "🔓"];
...
const SummaryCard: React.FC<{ idx: number; delay: number }> = ({ idx, delay }) => {
  ...
  return (
    <div style={{ ...a, width: 1460, display: "flex", alignItems: "center", gap: 24, padding: "22px 34px", borderRadius: RADIUS.xl, background: COLORS.surface, border: `2px solid ${accent}55`, boxShadow: SHADOW.lg }}>
      <div style={{ width: 64, height: 64, flexShrink: 0, borderRadius: "50%", background: `${accent}16`, border: `2px solid ${accent}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 30 }}>{EMOJIS[idx]}</div>
      <div style={{ width: 48, flexShrink: 0, fontFamily: FONT.ui, fontWeight: 800, fontSize: 50, color: accent, lineHeight: 1 }}>{idx + 1}</div>
      <span style={{ fontFamily: FONT.uiCjk, fontWeight: 700, fontSize: TYPE.h3, color: COLORS.ink, lineHeight: 1.32 }}>{OUTRO_CARDS[idx]}</span>
    </div>
  );
};
    ...
    <div style={{ ...a, display: "flex", flexDirection: "column", alignItems: "center", gap: 32 }}>
      ...
      <div style={{ marginTop: 14, display: "inline-flex", alignItems: "center", gap: 16, padding: "14px 30px", borderRadius: RADIUS.pill, background: COLORS.surface, border: `1px solid ${COLORS.border}`, boxShadow: SHADOW.sm }}>
        ...
        <span style={{ fontFamily: FONT.uiCjk, fontWeight: 800, fontSize: TYPE.h3, color: COLORS.ink }}>
          <span style={{ fontFamily: FONT.mono }}>Claude Code</span> 安裝經驗
        </span>
...
export const Outro: React.FC = () => {
  const frame = useCurrentFrame();
  const cardsLeave = leave(frame, at(5) - 6, 14);
  const headOut = leave(frame, at(5) - 6, 14);

  return (
    <Shell durationInFrames={DUR} showChrome={false} accent={MOTIF.install} seed="outro">
      <div style={{ position: "absolute", left: 0, right: 0, top: 86, display: "flex", justifyContent: "center", opacity: headOut }}>
        <div style={{ ...appearUp(frame, at(0), 18, 22), textAlign: "center" }}>
          <div style={{ fontFamily: FONT.uiCjk, fontWeight: 800, fontSize: TYPE.h1, letterSpacing: -1, color: COLORS.ink }}>四個經驗，總整理</div>
          <div style={{ marginTop: 8, fontFamily: FONT.mono, fontWeight: 500, fontSize: TYPE.small, color: COLORS.muted }}>The four lessons, recapped</div>
        </div>
      </div>
      ...
      {frame >= at(5) - 16 ? (
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", opacity: ramp(frame, at(5) - 10, at(5) + 8) }}>
          <EndCard delay={at(5)} />
        </div>
      ) : null}
      ...
      <Sfx src="whoosh" at={at(5) - 6} volume={0.34} />
      <Sfx src="pop" at={at(5) + 10} volume={0.34} />
      <Captions cues={CUES} />
    </Shell>
  );
};
