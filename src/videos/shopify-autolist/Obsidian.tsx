...
import spec from "../_explainer/specs/shopify-autolist.json";
import voManifest from "../_explainer/specs/shopify-autolist.vo.json";
import { WhiteBg, ACC, darkCard, Glare, hairline, emberClip, rgba, accOf } from "./glass";
import { ClaudeConnectors, ShopifyInventoryPanel } from "./ShopifyMock";

/**
 * shopify-autolist 黑曜石版（16:9, 1920×1080, 30fps）· 生活應用 EP08。
 * 資料驅動：讀 _explainer/specs/shopify-autolist.json（過稿版）+ 同名 vo.json + 曉晴 VO(public/vo/shopify-autolist)。
 * 視覺皮＝淺底畫布 + 黑曜石深卡玻璃 + 橙漸層鉤子 + 概念錨頂帶（系列一致，照 fb-autopost 範本）。
 */
const FPS = 30;
const VO = voManifest as Record<string, number>;
const HAS_VO = Object.keys(VO).length > 0; // vo.json 填入後自動啟用 VO 音軌
const HAS_BGM = true; // public/bgm-shopify-autolist.mp3（Gemini Lyria 水晶鋼琴 104.7s，Audio loop）
...
const ANCHORS: (React.ReactNode | null)[] = [
  ...
  <>先查重，<OS>有就沿用</OS></>,
  <>GEO = <OS>讓 AI 引用你</OS></>,
  <>Shopify 預設<OS>不追蹤庫存</OS></>,
  <>圖片是<OS>非同步</OS>的</>,
  <>驗收看 <OS>variant</OS> 的數字</>,
  <><OS>32 件</OS>商品實測</>,
  <>圈好<OS>邊界</OS>再自動化</>,
  null,
  null,
...
export const shopifyObsidianFrames = () =>
    ...
    <AbsoluteFill style={{ fontFamily: FONT.uiCjk, color: ACC.ink }}>
      <div style={{ position: "absolute", top: 62, left: 96, display: "flex", alignItems: "center", gap: 22, opacity: o }}>
        <div style={{ ...darkCard(ACC.ember, { r: 999, glow: 0.5 }), padding: "10px 28px", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 30, letterSpacing: 2, color: "#fff" }}>生活應用系列 EP08</span>
        </div>
      ...
      <div style={{ position: "absolute", top: 150, right: 92, width: 620, ...darkCard(ACC.ember, { r: 40, glow: 1.2 }), overflow: "hidden", padding: "40px 42px", transform: `rotate(-2.5deg)`, opacity: o }}>
        ...
        <div style={{ position: "relative", padding: "22px 24px", borderRadius: 16, background: rgba("#060a14", 0.7), border: `1px solid ${rgba(ACC.ember, 0.35)}`, boxShadow: `inset 0 2px 6px ${rgba("#000", 0.5)}` }}>
          <span style={{ fontFamily: FONT.monoCjk, fontSize: 30, lineHeight: 1.42, color: "#fff" }}>
            <span style={{ color: ACC.gold }}>$</span> 「Claude，幫我把這件商品<span style={{ color: ACC.emberSoft }}>上架到 Shopify</span>」
          </span>
        </div>
        ...
        <div style={{ position: "relative", display: "flex", flexDirection: "column", gap: 15 }}>
          {["查重 → GEO → 庫存 → CDN → 驗收", "官方 Shopify 連接器 · 0 行程式碼", "32 件實測 · 30 分鐘 → 3 分鐘"].map((t, i) => (
...
/** CH3/CH6：依真實 UI 重建的 mockup 場景（kicker + 標題 + 💬 prompt 藥丸 + 重建畫面 + keyline）。 */
const MockupScene: React.FC<{ s: Scene; f: number; showKey: boolean; mock: React.ReactNode; scale?: number }> = ({ s, f, showKey, mock, scale = 1 }) => {
  const c = accOf(s.accent);
  const prompt: string = s.terminal?.lines?.find((l: any) => l.k === "cmd")?.t ?? "";
          ...
          {s.nodes.map((n: any, i: number) => {
            ...
            return (
              <React.Fragment key={i}>
                <div style={{ ...darkCard(nc, { r: 24, glow: 0.9 }), position: "relative", overflow: "hidden", width: 280, minHeight: 210, padding: "28px 22px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 12, opacity: o, transform: `translateY(${(1 - o) * 18}px)` }}>
                  <Glare />
                  ...
                  {n.sub && <span style={{ position: "relative", fontWeight: 500, fontSize: 23, color: rgba("#ffffff", 0.66), lineHeight: 1.3 }}>{n.sub}</span>}
                </div>
                {i < s.nodes.length - 1 && <span style={{ fontSize: 42, fontWeight: 800, color: ACC.emberSoft, opacity: fadeIn(f, 14 + i * 8, 12) }}>→</span>}
              </React.Fragment>
            );
          })}
        </div>
    ...
    <AbsoluteFill style={{ alignItems: "center", justifyContent: "center", fontFamily: FONT.uiCjk }}>
      <div style={{ textAlign: "center", opacity: o }}>
        ...
        <div style={{ display: "flex", gap: 26, justifyContent: "center", marginTop: 44 }}>
          {[["👍", "按讚", ACC.mint], ["🔔", "訂閱", ACC.rose], ["🔗", "分享", ACC.shop]].map(([e, t, c], i) => (
        ...
        <div style={{ marginTop: 44, display: "inline-flex", alignItems: "center", gap: 14, ...darkCard(ACC.ember, { r: 999, glow: 0.5 }), padding: "12px 30px" }}>
          ...
          <span style={{ fontWeight: 500, fontSize: 26, letterSpacing: 3, color: rgba("#ffffff", 0.6) }}>· 生活應用系列 EP08 · {spec.brand.date.split("·").pop()?.trim()}</span>
        </div>
      </div>
    </AbsoluteFill>
  );
...
const SceneShell: React.FC<{ idx: number }> = ({ idx }) => {
  ...
  else if (idx === 3) visual = <MockupScene s={s} f={f} showKey={f >= keyStart} mock={<ClaudeConnectors />} />;
  else if (idx === 6) visual = <MockupScene s={s} f={f} showKey={f >= keyStart} mock={<ShopifyInventoryPanel />} scale={0.94} />;
      ...
      {HAS_VO &&
        marks.map((m: any) => (
          <Sequence key={`a-${m.id}`} from={m.start}>
            <Audio src={staticFile(`vo/shopify-autolist/${m.id}.mp3`)} />
          </Sequence>
        ))}
    </AbsoluteFill>
  );
...
export const ShopifyAutolistThumb: React.FC = () => (
...
export const ShopifyAutolistObsidian: React.FC<{ bgm?: boolean }> = ({ bgm = true }) => (
  // textSpacingTrim off：Chrome 的 CJK 標點擠壓在「全形標點相鄰但分屬不同字型分片」時
  // （本片 v1 禮物景「都查），有」行）會逐 render-worker 不一致 → 整行半字寬左右跳。
  <AbsoluteFill style={{ textSpacingTrim: "space-all" }}>
    <WhiteBg />
    {bgm && HAS_BGM && <Audio loop src={staticFile("bgm-shopify-autolist.mp3")} volume={0.11} />}
    <TransitionSeries>
      {SCENES.flatMap((s, i) => {
        const seq = (
