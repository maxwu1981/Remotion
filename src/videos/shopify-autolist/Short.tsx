...
export const shopifyShortFrames = () => hookDur + promptDur + CTA - 2 * XF;
    ...
    <AbsoluteFill style={{ fontFamily: FONT.uiCjk, alignItems: "center", justifyContent: "flex-start", paddingTop: 190 }}>
      <div style={{ opacity: o, transform: `translateY(${(1 - o) * 18}px)`, textAlign: "center" }}>
        <div style={{ ...darkCard(ACC.ember, { r: 999, glow: 0.5 }), display: "inline-flex", alignItems: "center", gap: 12, padding: "12px 30px", marginBottom: 40 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 34, letterSpacing: 3, color: "#fff" }}>生活應用系列 EP08</span>
        </div>
        <div style={{ fontFamily: FONT.monoCjk, fontWeight: 700, fontSize: 84, color: ACC.ink }}>Shopify</div>
        <div style={{ ...emberClip, background: "linear-gradient(160deg,#F09A6B,#D97757 45%,#B54C29)", WebkitBackgroundClip: "text", backgroundClip: "text", fontWeight: 800, fontSize: 168, letterSpacing: -3, lineHeight: 1.02 }}>一句話<br />自動上架</div>
      </div>
      {/* 4 chips 2x2 */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 22, marginTop: 70, opacity: o }}>
        {[["1", "句話上架", "claude"], ["3", "分鐘一件", "green"], ["0", "行程式碼", "blue"], ["32", "件商品實測", "violet"]].map(([n, t, a], i) => {
          const c = accOf(a);
          return (
...
const PROMPT_LINES = [
  "① 查重：同名商品先查，有就沿用絕不重建",
  "② GEO 建商品：英文關鍵字標題＋SEO＋中英 tags",
  "③ 價格：建好後停下來讓我確認",
  "④ 庫存三件套：tracked＋DENY＋qty=1",
  "⑤ 圖片：輪詢等 cdn.shopify.com 正式網址",
  "⑥ 驗收：庫存／政策／網址逐項回報",
];
    ...
    <AbsoluteFill style={{ fontFamily: FONT.uiCjk, alignItems: "center", justifyContent: "center", padding: "0 60px" }}>
      ...
      <div style={{ ...darkCard(ACC.ember, { r: 24, glow: 0.95 }), position: "relative", overflow: "hidden", width: "100%", padding: "34px 36px", opacity: o, transform: `translateY(${(1 - o) * 16}px)` }}>
        ...
        <div style={{ position: "relative", fontFamily: FONT.monoCjk, fontSize: 32, color: "#fff", marginBottom: 20 }}>
          <span style={{ color: ACC.gold }}>$</span> 「我的 Shopify 已接上你，幫我上架這件商品：」
        </div>
...
export const ShopifyAutolistShort: React.FC = () => (
  <AbsoluteFill style={{ textSpacingTrim: "space-all" }}>
    <WhiteBg />
    <Audio loop src={staticFile("bgm-shopify-autolist.mp3")} volume={0.13} />
    <Sequence from={LEAD} durationInFrames={f2("cv1")}><Audio src={staticFile("vo/shopify-autolist/cv1.mp3")} /></Sequence>
    <Sequence from={hookDur - XF + LEAD} durationInFrames={f2("gp2")}><Audio src={staticFile("vo/shopify-autolist/gp2.mp3")} /></Sequence>
    <TransitionSeries>
      <TransitionSeries.Sequence durationInFrames={hookDur}>
        <><HookScene /><Cap text="上架一件商品，一句話 3 分鐘（手動要 30 分）" from={LEAD} dur={f2("cv1")} /></>
      ...
      <TransitionSeries.Sequence durationInFrames={promptDur}>
        <><PromptScene /><Cap text="每一步合成一段完整 prompt，貼給 Claude 一次上好" from={LEAD} dur={f2("gp2")} /></>
      </TransitionSeries.Sequence>
      <TransitionSeries.Transition presentation={fade()} timing={linearTiming({ durationInFrames: XF })} />
